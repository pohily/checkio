def unfair_districts(amount_of_people, grid):
    ...
    while True:                   #if found zoning fail polls, start new zoning
        ...
        while len(used_districts) < dis_number:
                    ...
                    for c in cross:
                        ...
                        if 0 <= x+i <= v and 0 <= y+j <= h:
                            if (x+i, y+j) not in used_districts and current_population + grid_pop[x+i][y+j] <= amount_of_people:
                                ...
                                to_look_check = True
                if to_look_check == False: # check if search stuck
                    failed += current_zones + [current_zone]
                    break
                if to_look:
                    i, j = to_look.pop(0)
            ...
            if current_zone not in failed:          #check for fail
        ...
        for i, zone in enumerate(current_zones):
            ...
            if f > s:
                ...
                winners.append('s')
        if winners.count('f') > winners.count('s'):
            return grid_zones
        else:
            failed += current_zones
